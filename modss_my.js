if (!window.loaded_modss) {
	...
	(function () {
		...
		console.log("Modss", "init", "LOADED - RU [77.51.125.202] - [GET] " + url);
		...
		network.silent(
			...
			function (json) {
				try {
					...
					console.log("Modss полученный код с сервера:", json);
			...
			function (a, c) {
				console.log("Modss", "init", "RU [77.51.125.202] - load get plugin error - [POST] " + url, network.errorDecode(a, c));
				Lampa.Noty.show("RU [77.51.125.202] - [POST] { " + url + " } | MODSs ОШИБКА ЗАГРУЗКИ ПЛАГИНА -> причина: " + network.errorDecode(a, c));
